...
**符號選擇說明**: 實際交易標的選用 **VOO**(Vanguard S&P 500 ETF) 而非 `exp_007` 驗證用的 SPY(State Street SPDR S&P 500 ETF) — 兩者追蹤同一指數(S&P 500), 日報酬率歷史相關係數 > 0.999, 差異主要在費用率與稅務結構, 對趨勢跟蹤策略的訊號與績效影響可忽略. 這代表嚴格意義上 `exp_002` 沒有直接在 VOO 的價格數據上跑過樣本外驗證, 只是「高度相似標的的驗證結果」, 不是「VOO 本身的驗證結果」— 記錄於此, 不誇大為已驗證.

## 目標與範圍 (goal and scope)

**目標**: 新增一條獨立的美股紙上交易 pipeline(VOO + QQQ, Alpaca Paper Trading), 重用既有的 `signal_agent` / `risk_agent` 決策邏輯與凍結的 `exp_002` 參數, 每個交易日收盤後執行一次, 用 limit-on-open (LOO, 開盤限價) 委託單完成次日開盤附近的進場保護.

**本切片範圍內**:

- 交易標的: `["VOO", "QQQ"]`, 皆屬 `risk_agent.SYMBOL_MARKET_TYPES` 的 `"stocks"` 類別
...
- VOO/QQQ 以外的美股標的(架構上與加密貨幣 Slice 2 擴展 ETH 的方式相同, 之後只需在 `SYMBOLS` 與 `SYMBOL_MARKET_TYPES` 加一行)
...
- `04_paper_trading/alpaca_paper_trading_client.py` — 對應 `binance_testnet_client.py`. 用 `APCA-API-KEY-ID` / `APCA-API-SECRET-KEY` 標頭驗證(比 Binance 的 HMAC 簽名簡單, 不需組簽名字串). 提供:
  ...
  - `get_todays_calendar_entry(today: str) -> dict | None`: 查 `/v2/calendar?start=today&end=today`(`today` 為呼叫端算好的美東日期字串, 見下方資料流第 1 步), 無資料代表今天非交易日
...
- `04_paper_trading/agents/risk_agent.py` — `SYMBOL_MARKET_TYPES` 新增 `"VOO": "stocks", "QQQ": "stocks"` 兩個條目
...
1. **交易日曆檢查**: 以美東時間算出今天日期字串, 呼叫 `alpaca_paper_trading_client.get_todays_calendar_entry(today)`. 若為 `None`(週末/假日), 記錄一筆 `{"market_open": false}` 的 no-op 執行紀錄, 直接結束, 不產生信號, 不查帳戶, 不發 Telegram
...
5. **單一 `risk_agent.review_portfolio(...)` 呼叫**: 與加密貨幣側完全相同的函式與規則順序(全域熔斷 → 逐標的數據異常 → 逐標的目標倉位比對 → 開倉四項檢查), 差別只在傳入的 `SYMBOLS`/`market_type` 落在 `"stocks"` 類別、`max_positions_by_market["stocks"]=5`. **預期會頻繁觸發的情境**: VOO 與 QQQ 日報酬率歷史相關係數通常 > 0.8, 兩者同時從空手轉多單時, 依固定順序先核准的一個會讓另一個在相關性檢查被拒 — 這是既有規則的既定行為, 不是本切片的缺陷(與加密貨幣側 Slice 2 設計文件記錄的 BTC/ETH 情境完全對稱)
...
- **相關性限制擋下 VOO/QQQ 同時開倉**: 預期行為, 見資料流第 5 步說明, 不是需要修的錯誤
...
- `risk_agent.py`: 擴充既有 `tests/test_paper_trading_risk_agent.py`, 新增 VOO/QQQ 屬於 `"stocks"` 類別、`max_positions_by_market["stocks"]` 生效的案例
...
04_paper_trading/
  ...
  agents/
    ...
    risk_agent.py                    (修改: SYMBOL_MARKET_TYPES 新增 VOO/QQQ)
  logs/                              (gitignore 排除)
