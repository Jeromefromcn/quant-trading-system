...
回測引擎的不變量測試(tests/test_engine_invariants.py) 只能證明引擎與輸入數據一致(verification) ,
無法證明輸入數據本身符合真實市場. 若抓取器把欄位對錯, 所有不變量照樣全綠, 數字卻系統性錯誤.
本腳本把 Binance 快取的 BTC 日線, 拿去和完全獨立的 Kraken 交易所日線逐日比對收盤價, 提供那一個外部錨.

這是每個數據源做一次的驗證, 不是每次跑都要做, 也不放進 pytest(需要網路且依賴外部服務) .
