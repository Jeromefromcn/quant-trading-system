...
price_dataframe['candle_direction_apply_method'] = price_dataframe.apply(classify_candle, axis=1)
print(price_dataframe['candle_direction_apply_method'].value_counts())

# ── 向量化等效寫法(更快) ────────────────────────────────────────────────────

price_dataframe['candle_direction_vectorized_method'] = np.where(price_dataframe['close'] > price_dataframe['open'], 'bullish', 'bearish')

# 驗證結果一致
assert (price_dataframe['candle_direction_apply_method'] == price_dataframe['candle_direction_vectorized_method']).all(), "結果不一致! "
...
print(f"向量化快了 {apply_method_time_ms / max(vectorized_method_time_ms, 0.001):.0f} 倍")
...
price_dataframe['volume_ratio'] = price_dataframe['volume'] / price_dataframe['volume_moving_average_20_day']
price_dataframe = price_dataframe.dropna()

price_dataframe['market_regime_apply_method'] = price_dataframe.apply(classify_market_regime, axis=1)
...
price_dataframe['market_regime_vectorized_method'] = np.select(conditions, choices, default='normal')

assert (price_dataframe['market_regime_apply_method'] == price_dataframe['market_regime_vectorized_method']).all()
...
column_statistics = price_dataframe[['daily_return', 'volume_ratio']].apply(compute_statistics_summary)
...
print(price_dataframe[['daily_return', 'signal', 'signal_description']].head(8).round(4))
