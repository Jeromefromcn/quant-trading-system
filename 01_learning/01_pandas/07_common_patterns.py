...
price_dataframe['rolling_5_day_high'] = price_dataframe['high'].rolling(rolling_window_size).max()
...
price_dataframe['volume_size_category'] = np.where(price_dataframe['volume'] > price_dataframe['volume'].mean(), 'large', 'small')
...
backtest_preparation_dataframe = backtest_preparation_dataframe.dropna(subset=['moving_average_20_day'])  # 只丟棄 moving_average_20_day 是 NaN 的行
...
price_dataframe['next_close'] = price_dataframe['close'].shift(-1)  # 這是明日收盤, 只能用於分析, 不能用於生成信號
...
print(f"series_one + series_two(index 不對齊會產生 NaN) :\n{index_aligned_addition_result}")
