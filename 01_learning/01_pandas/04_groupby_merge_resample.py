...
merged = df_with_date.merge(df_sentiment, on='date', how='left') # left join 保留所有日線
...
print("\n=== resample 日線轉周線 ===")
weekly = df_daily['close'].resample('W').agg(
 open=('first'),
 high=('max'),
 low=('min'),
 close=('last'),
...
result = pd.merge_asof(
 ...
 df_weekly_reset[['date', 'weekly_return']].rename(columns={'weekly_return': 'w_ret'}),
 on='date',
