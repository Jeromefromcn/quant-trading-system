...
df_daily = pd.DataFrame(
    ...
    index=daily_trading_date_index,
)
...
# 多種聚合方式
print("\n=== groupby 多聚合 ===")
weekly_statistics = df_daily.groupby("week")["close"].agg(["min", "max", "mean", "std"])
print(weekly_statistics.round(2))
...
print("\n=== pd.concat 縱向拼接 ===")
january_price_dataframe = df_daily[df_daily["month"] == 1].copy()
february_price_dataframe = df_daily[df_daily["month"] == 2].copy()
...
print("\n=== resample 日線轉月線 OHLCV(Open, High, Low, Close, Volume) ===")
monthly_price_volume_dataframe = df_daily.resample("ME").agg(
    {
        "close": ["first", "max", "min", "last"],
        "volume": "sum",
    }
)
monthly_price_volume_dataframe.columns = ["open", "high", "low", "close", "volume"]
print(monthly_price_volume_dataframe.round(2))
...
result = pd.merge_asof(
    ...
    df_weekly_reset[["date", "weekly_return"]].rename(
        columns={"weekly_return": "weekly_return_value"}
    ),
    on="date",
