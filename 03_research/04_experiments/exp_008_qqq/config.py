"""
exp_008 跨市場驗證 — 凍結的 exp_002 (best-so-far) 搬到美股 QQQ (那斯達克 100), 檢驗優勢能否跨資產類別泛化
策略參數與 exp_002 逐項相同, 只換數據為 QQQ; 引擎唯一調整 trading_days_per_year=252 (美股年化正確性修正, 非調參)
費率故意維持 Binance 0.1% (對股票偏保守/高估成本; 若在高估成本下仍成立則證據更強)
不得為 QQQ 調任何策略參數 (調了就把這個乾淨市場燒成驗證集). exp_006-008 之一
執行: 直接 python config.py (資料夾名字自動當實驗名), 或 python ../run_experiment.py exp_008_qqq
"""

# 數據: QQQ 日線 (至今零實驗的乾淨市場, 那斯達克 100 ETF, 偏科技成長股)
DATASET = "qqq_1d.csv"
