...
Ooi, Pedersen 2012) 的標準做法: 落後 365 天 (加密貨幣年化慣例, 對應「過去 12 個月」) 報酬為正則
持有多單, 否則空手 (long-only, 呼應 exp_002 現貨不放空) . 這與 exp_002 用的 EMA 交叉 + ADX 完全
是不同的訊號建構, 是真正獨立的「通用趨勢」代理, 不是 exp_002 自己的訊號改個名字.
...
# 通用動能因子的落後窗口: 365 天呼應 TSMOM 文獻慣用的「過去 12 個月」, 且與本專案加密貨幣年化天數一致
...
def run_ols_with_newey_west(design_matrix: np.ndarray, target: np.ndarray, lag: int) -> dict:
    ...
    # u_t = x_t * e_t (逐筆的「分數」向量), HAC 的核心是這些分數的自我與跨期共變異數
...
def _print_regression_report(
    ...
    print(
        f"\n  alpha 變化: {_annualize_daily_alpha(alpha):+.2%} (p={p_alpha:.3f}) "
        f"→ {_annualize_daily_alpha(alpha_2f):+.2%} (p={p_alpha_2f:.3f}) 加入動能因子後"
    )
...
def main() -> None:
    ...
    # 官方樣本外 (OOS) 穩健性檢查: 對齊 run_with_split 的官方切分, 呼應本專案「OOS 數字才算數」的慣例
