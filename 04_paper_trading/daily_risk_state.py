"""
每日風控狀態: 記錄今天(UTC) 開始時的帳戶淨值, 供每日虧損熔斷(daily circuit breaker) 規則
計算當日累計虧損用. `run_once.py` 刻意保持無狀態(每次都查詢交易所真實狀態, 不信任本地記憶) ,
但交易所現貨帳戶沒有今天損益這種端點可查, 這份小型本地快取只補這一個交易所查不到的基準點,
本身不是真相來源: 真相來源永遠是查詢到的當前淨值, 這份檔案只回答要跟哪一個基準比
"""
import json
import os
...
def load_daily_state(file_path: str) -> dict:
    """
    讀取每日風控狀態檔; 檔案不存在或內容無法解析時, 回傳空字典(呼叫端會視為尚無基準, 直接重置) ,
